...
async def admin(bot, message):
    if len(message.command) == 2:
        ...
        try:
            await bot.promote_chat_member(message.chat.id, user_id, can_change_info=True, can_invite_users=True,
...
async def demote(bot, message):
    if len(message.command) == 2:
        user_id = message.command[1]
        try:
            await bot.promote_chat_member(message.chat.id, user_id, can_change_info=False, can_invite_users=False,
